# Git Workflow for plugin-clean

This repository contains the Slidebolt Clean Plugin, which provides a clean starting point for new plugins and seeds demo entities. It produces a standalone binary.
...
- **Artifacts:** Produces a binary named `plugin-clean`.
- **Command:** `go build -o plugin-clean ./cmd/plugin-clean`
- **Validation:** 
  - Validated through unit tests: `go test -v ./...`
  - Validated through BDD tests: `go test -v ./cmd/plugin-clean`
  - Validated by successful compilation of the binary.

## Pre-requisites & Publishing
As a base plugin implementation, `plugin-clean` must be updated whenever the core domain, messaging, storage, or testkit SDKs are changed.

**Before publishing:**
1. Determine current tag: `git tag | sort -V | tail -n 1`
2. Ensure all local tests pass: `go test -v ./...`
3. Ensure the binary builds: `go build -o plugin-clean ./cmd/plugin-clean`

**Publishing Order:**
1. Ensure all internal dependencies are tagged and pushed.
2. Update `plugin-clean/go.mod` to reference the latest tags.
3. Determine next semantic version for `plugin-clean` (e.g., `v1.0.4`).
...
1. **Modify:** Update demo entities or base logic in `app/` or `internal/`.
2. **Verify Local:**
   - Run `go mod tidy`.
   - Run `go test ./...`.
   - Run `go test ./cmd/plugin-clean` (BDD features).
   - Run `go build -o plugin-clean ./cmd/plugin-clean`.
